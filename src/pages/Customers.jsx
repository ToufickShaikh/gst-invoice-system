...
import Layout from '../componen    } catch (error) {
setGstinValid(false)
...
if (error.response?.status === 404) {
  ...
  toast.error('GST verification service error. Please try again or enter details manually.')
} else if (error.code === 'NETWORK_ERROR' || error.message.includes('Network Error')) {
  toast.error('Network error. Please check your connection and try again.')
} else {
  toast.error('GSTIN validation failed. Please check and enter details manually.')
}
    } finally {ort Table from '../components/Table'
